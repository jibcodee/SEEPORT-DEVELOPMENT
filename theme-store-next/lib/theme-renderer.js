...
function renderTheme(themeConfig, rootElement = document.documentElement) {
  ...
  const cardVal = colors.cardBackground || colors['--card-bg'] || colors['--surface'] || '#1E293B';
  ...
  // Manage physical background element for animations
  let animDiv = rootElement.querySelector('.tray-bg-animation');
  ...
  if (animDiv) {
    ...
    if (document && document.body) {
      document.body.className.split(' ').forEach(cls => {
        if (cls.startsWith('has-animation-') || cls.startsWith('theme-active-')) {
    ...
    if (animType !== 'none') {
      ...
      } else if (animType === 'gif-loop' && animUrl !== 'none') {
